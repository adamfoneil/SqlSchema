        ...
        private async Task AddTablesAsync(IDbConnection connection, List<DbObject> results)
        {
            ...
            var columns = await connection.QueryAsync<Column>(
                @"WITH [pkColumns] AS (
	                ...
	                CASE
		                WHEN TYPE_NAME([col].[user_type_id]) LIKE 'nvar%' AND [col].[max_length]>0 THEN ([col].[max_length]/2)
		                WHEN TYPE_NAME([col].[user_type_id]) LIKE 'nvar%' AND [col].[max_length]=0 THEN -1
		                ELSE [col].[max_length]
	                END AS [MaxLength],
